...
func main() {
	arr := os.Args[1:]
	if len(arr) != 9 { //check for quantity of arguments
		fmt.Println("Error")
		return
	}

	for _, arg := range arr {
		if len(arg) != 9 { // check gor quantity of symbols in each argument
			fmt.Println("Error")
			return
		}

		for _, ch := range arg { // check for numbers or "."
			if ch < '0' || ch > '9' {
				if ch != '.' {
					fmt.Println("Error")
					return
